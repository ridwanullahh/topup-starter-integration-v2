          ...
          <Button
            ...
            <div className="flex items-center space-x-3 pr-2">
              ...
              <div className="flex items-center space-x-3">
                <span className={cn("inline-block truncate text-sm font-medium xl:max-w-[120px]", large ? "w-full" : "max-w-[80px]")}>{selected.slug}</span>
              </div>
            </div>
            <ChevronsUpDown
...
function ProjectList({ selected, projects, setOpenPopover }: { selected: ProjectType; projects: ProjectType[]; setOpenPopover: (open: boolean) => void }) {
  return (
    <div className="flex flex-col gap-1">
      {projects.map(({ slug, color }) => (
        <Link
          key={slug}
          className={cn(buttonVariants({ variant: "ghost" }), "relative flex h-9 items-center gap-3 p-3 text-muted-foreground hover:text-foreground")}
          href="#"
          onClick={() => setOpenPopover(false)}>
          <div className={cn("size-3 shrink-0 rounded-full", color)} />
          <span className={`flex-1 truncate text-sm ${selected.slug === slug ? "font-medium text-foreground" : "font-normal"}`}>{slug}</span>
          {selected.slug === slug && (
